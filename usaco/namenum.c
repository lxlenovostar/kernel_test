...
int get_num(char *tmp)
{
		 ...
		 * 注意这里是列是4，因为包括一个'\0'
		 */
		char keyvalue[8][4] = {"ABC", "DEF", "GHI", "JKL", "MNO", "PRS", "TUV", "WXY"};
		...
		for(i = 0; i < strlen(tmp); ++i){
			for(j = 0; j < 8; ++j){
				if(strchr((char const *)(keyvalue + j), (int)(*(tmp + i))) != NULL){
						index[i] = j + 2;
						//printf("keyvalue is %s\n", (keyvalue + j));
						//printf("%c and %d find and i is %d\n", *(tmp + i), index[i], i);
						break;
				}
			}
		}
...
main()
{
	...
	while( fgets(buffer, NUM, fdict) != NULL){
		...
		len = (int)strlen(temp);
		struct inode *ptr = (struct inode*)malloc(sizeof(struct inode));	
	...
	list_for_each_entry_safe(at, tmp, &head_list[index], list){
		if (get_num(at->name) == key){
